...
static int call_minicore(long signr);
...
minicore_launch(long signr, siginfo_t *info)
{
	...
	printk(KERN_INFO "%s: CRASH! %s(%d) received %ld. "
			"Waiting up to 5s for minicore to attach.\n",
			__FUNCTION__, current->comm, current->tgid, signr);

	DBG("%d/%d wait_ptrace\n", current->pid, current->tgid);

	/* Wait for minicore to ptrace attach */
	timeleft =
		wait_for_completion_timeout(current->ptrace_attach_done, 5*HZ);
...
static void
argv_cleanup(char **argv, char **envp)
{
	argv_free(argv);
}

static int
call_minicore(long signr)
{
	...
	lock_kernel();
	is_handled = format_corename(corename, minicore_pattern, signr);
	unlock_kernel();
