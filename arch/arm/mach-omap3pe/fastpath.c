...
/* This is the only arch specific cheat...*/
...
static int rtc_alarm_write(struct rtc_device *rtc, unsigned long next_alarm)
{
	...
	DEBUG("%s: Setting rtc clock to %02d:%02d:%02d\n", __FUNCTION__,
		alm.time.tm_hour, alm.time.tm_min, alm.time.tm_sec);
...
int fastpath_prepare(void)
{
	...
	/* Read if last battery reading was a while ago... */
	...
	DEBUG("%s: next battery check is in %ds\n", __FUNCTION__, battery_poll_next_seconds()); 
...
/** 
* @brief Return 1 if the wakeup source is solely RTC.
* 
* @retval
*/
static int wakeup_source_is_rtc(void)
{
	return omap3_wakeup_sources_get() == WAKEUP_SOURCE_RTC;
}
...
int fastpath_fastsleep(void)
{
	...
	if (FASTPATH_STATE_RTC_SYSTEM == fastpath_state && wakeup_source_is_rtc()) {
