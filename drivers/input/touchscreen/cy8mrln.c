		...
		case CY8MRLN_IOCTL_GET_QUERY_DATA:
		{
			...
			if(CY8MRLN_IOCTL_GET_QUERY_DATA == cmd) {
				...
				*/
				...
				printk(KERN_INFO "%s: Got IDAC table\n", DRIVER);

				if (copy_to_user(usr_ptr, &dev->rev[79],
						 usr_bytes)) {
					rc = -EFAULT;
					goto Done;
				}
...
cy8mrln_probe(struct spi_device *spi)
{
	...
	rc = gpio_direction_output(dev->pdata->enable_gpio, 0);
	if (rc != 0) {
