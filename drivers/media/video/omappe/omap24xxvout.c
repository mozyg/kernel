...
static int rotation_support = 0;//-1;
		       ...
		       unsigned int cmd, void *arg)
{
	...
	case VIDIOC_REQBUFS:
	{
		...
		/*
		 * we will NOT allow re-requests to allocate buffers.
		 *
		 */
		return -ENOMEM;
	...
	case VIDIOC_QBUF:
	{
		...
		if (vout->rotation >= 0){
			...
			vout->last_videobuff_dma = videobuf_to_dma(q->bufs[buffer->index])->bus_addr;
			omap_set_dma_src_params (vout->vrfb_dma_tx.dma_ch,
				0,	// src_port required only for OMAP1
				OMAP_DMA_AMODE_POST_INC, vout->last_videobuff_dma ,
			...
			if (vout->mirror == 1){
				...
				vout->last_videobuff_dma = vout->sms_rot_phy[vout->pos][0];			
				omap2_disp_start_vlayer (vout->vid,&vout->pix, &vout->crop, &vout->win,
						vout->last_videobuff_dma, mir_rot_deg, vout->mirror);
			}
			else{
				vout->last_videobuff_dma = vout->sms_rot_phy[vout->pos][0];
				omap2_disp_start_vlayer (vout->vid,&vout->pix,&vout->crop, &vout->win,
				vout->last_videobuff_dma, vout->rotation, vout->mirror);
			}
		}/*if rotation */
		else{
			vout->last_videobuff_dma = videobuf_to_dma(q->bufs[buffer->index])->bus_addr;
		...
		if (vout_linked != -1){
			...
			if (dest->streaming){
				if (dest->rotation >= 0){
					...
					videobuf_to_dma(q->bufs[buffer->index])->bus_addr, dest->rotation,
					dest->mirror);
	...
	case VIDIOC_STREAMON:
	{
		...
		vout->streaming = fh;
		vout->pos	= 0;
		vout->last_videobuff_dma = (dma_addr_t) NULL;
...
omap24xxvout_mmap (struct file *file, struct vm_area_struct *vma)
{
	...
	pos = videobuf_to_dma(q->bufs[i])->vmalloc;
...
static int
dummy_vbq_setup(struct videobuf_queue *q, unsigned int *count,
		unsigned int *size)
{
	BUG();
	return (0);
...
static int
dummy_vbq_prepare(struct videobuf_queue *q, struct videobuf_buffer *qbuf,
...
static void
dummy_vbq_queue(struct videobuf_queue *q, struct videobuf_buffer *qbuf)
{
	BUG();
}
...
omap24xxvout_open (struct inode *inode, struct file *file)
{
	...
	q = &fh->vbq;
	dummy_vbq_ops.buf_setup = dummy_vbq_setup;
	...
	for (i = 0; i < vout->buffer_allocated; i++) {
		...
		videobuf_to_dma(q->bufs[i])->vmalloc = (void *) vout->buf_virt_addr[i];
		videobuf_to_dma(q->bufs[i])->bus_addr = vout->buf_phy_addr[i];
...
static int init_vout_memory(struct omap24xxvout_device *vout,
				u32 memwidth,
				u32 memheight,
				u32 membytes_per_pixel)
{
	/* initialize memory for vout device */
	/* we use V2out for video.  V1out is not used.
	 * because of memory limitations, we will pre-allocate the buffers
	 * here so that we don't fail when we are in low memory situations
	 */
	...
	if (vout == NULL) {
		printk("%s: Error:  vout device NULL\n", __FUNCTION__);
		return -EINVAL;
	}

	/* Only 2 buffers will be allocated at this point.  We don't need
	 * more than 2, not at this level.  THis level is also not
	 * not exposed to 3rd party developers, so we should be safe.
	 */
	if ((memwidth == 0) && (memheight == 0)) {
		/* height and width not specified, use panel size */
		omap2_disp_get_panel_size(OMAP2_OUTPUT_LCD, &width, &height);

		vout->buffer_size = PAGE_ALIGN(width * height * 2);
	} else {
		/* use specified height and width */
		vout->buffer_size = PAGE_ALIGN(memwidth * memheight *
							membytes_per_pixel);
	}

	for (i = 0; i < MAX_ALLOWED_VIDBUFFERS; i++) {
		DPRINTK("%s: allocating %d bytes for buf %d\n", __FUNCTION__,
			vout->buffer_size, i);
		vout->buf_virt_addr[i] = (unsigned int)dma_alloc_coherent(
			NULL,
			vout->buffer_size,
			(dma_addr_t *)&vout->buf_phy_addr[i],
			GFP_KERNEL | GFP_DMA);
		...
		if (!vout->buf_virt_addr[i]) {
			printk("%s: Failed to alloc %d buf memory\n",
				__FUNCTION__, i);
			break;
		}

		memset((void *)vout->buf_virt_addr[i], 0, vout->buffer_size);

		DPRINTK("REQBUFS: buffer %d: virt=0x%lx, phy=0x%lx, size=0x%x\n",
			i, vout->buf_virt_addr[i], vout->buf_phy_addr[i],
			vout->buffer_size);

		vout->buffer_allocated++;
	}

	/* need to double check if memory allocations are ALL successful */
	if (i != MAX_ALLOWED_VIDBUFFERS) {
		/* did not match, not all memory allocation was successful */
		i--;
		for(j=0; j<i; j++) {
			dma_free_coherent(NULL,
					vout->buffer_size,
					(void *) vout->buf_virt_addr[j],
					(dma_addr_t)vout->buf_phy_addr[j]);
		}
		vout->buffer_allocated = 0;
		vout->buffer_size = 0;

		printk("%s: video buf allocation failed\n", __FUNCTION__);
		return -ENOMEM;
	}

	vout->buf_memory_type = V4L2_MEMORY_MMAP;
	return 0;
}

static int init_vrfb_memory (struct omap24xxvout_device *vout,int width, int height)
{
	...
	if (vout->vid != vout_linked){
		if (vout->rotation >= 0){
			...
			if (vout->smsshado_size && vout->smsshado_size < vout->tile_aligned_psize) {
				DPRINTK("%s: have to free and re-allocate\n", __FUNCTION__);
			...
			* the image data and read the data requested space (rotation degree)
			 */
			DPRINTK("%s: Allocate the actual physical space for VRFB\n", __FUNCTION__);
			for(k = 0; k < 2; k++){
...
init_vout_device (int vid)
{
	...
	/* struct platform_device *this_dev; // defined in board file */
...
static int
omap24xxvout_probe (struct platform_device *pdev)
{
	struct vout_mem_alloc_data *vout_mem_plat;
	u32 width = 0;
	u32 height = 0;
	u32 bpp = 0;
	...
	if (pdev->dev.platform_data) {
		vout_mem_plat = (struct vout_mem_alloc_data*)
			pdev->dev.platform_data;

		width = vout_mem_plat->mem_width;
		height = vout_mem_plat->mem_height;
		bpp = vout_mem_plat->mem_bytes_per_pixel;
	}

	if (pdev->id == 11) {	// video1out
		saved_v1out = init_vout_device(OMAP2_VIDEO1);
		if (saved_v1out == NULL) {
			omap2_disp_put_dss();
			return -ENODEV;
		}
		// success - initialize memory */
		if (0 != init_vout_memory(saved_v1out, width, height, bpp)) {
			printk("%s: failed to initizlied vid1 memory\n",
				__FUNCTION__);
			cleanup_vout_device (OMAP2_VIDEO1);
			return -ENOMEM;
		}
		if ( 0 != init_vrfb_memory (saved_v1out,width, height)) {
			printk("%s: failed to initialize VRFB for vid1 memory\n",
				__FUNCTION__);
			cleanup_vout_device (OMAP2_VIDEO1);
			return -ENOMEM;
		}

		platform_set_drvdata (pdev, saved_v1out);

	} else if (pdev->id == 12) {	// video2out
		...
		// success - initialize memory */
		if (0 != init_vout_memory(saved_v2out, width, height, bpp)) {
			printk("%s: failed to initizlied vid1 memory\n",
				__FUNCTION__);
		...
		if ( 0 != init_vrfb_memory (saved_v2out,width, height)) {
			printk("%s: failed to initialize VRFB for vid1 memory\n",
				__FUNCTION__);
			cleanup_vout_device (OMAP2_VIDEO2);
			return -ENOMEM;
		...
		platform_set_drvdata (pdev, saved_v2out);

	} else {
		printk("%s:  ERROR, unknown device id %d\n",
			__FUNCTION__, pdev->id);
		return -EINVAL;
	}
...
omap24xxvout_init (void)
{
	...
	/* video1 */
	if (platform_driver_register(&omap24xxv1out_driver) != 0){
		printk(KERN_ERR VOUT_NAME": could not register Video1 driver\n");
		return -ENODEV;
	}

	/* register video2 */
	if (platform_driver_register(&omap24xxv2out_driver) != 0){
		printk(KERN_ERR VOUT_NAME": could not register Video2 driver\n");
		return -ENODEV;
	}

	/* save the register state(s) */
...
omap24xxvout_cleanup (void)
{
	...
	platform_driver_unregister(&omap24xxv1out_driver);
	platform_driver_unregister(&omap24xxv2out_driver);
}
