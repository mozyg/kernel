...
static int memnotify_enter_thresholds_size = 4;
...
static unsigned int lowmemnotify_poll(struct file *file, poll_table *wait)
{
	...
	if (changed) {
		...
		info->last_threshold = last_threshold;
 ...
 */
static ssize_t
meminfo_show(struct class *class, char *buf)
{
	unsigned long used;
	...
	len += snprintf(buf+len, PAGE_SIZE, "Enter Thresholds:\n");
	for (i = 0; i < memnotify_enter_thresholds_size; i++) {
		...
		len += snprintf(buf+len, PAGE_SIZE, "%s: %d, %ldMB, Rem: %ldMB:\n",
			threshold_string(i),
			memnotify_enter_thresholds[i],
			MB(limit),
			MB(left));
	}
	len += snprintf(buf+len, PAGE_SIZE, "Leave Thresholds:\n");
...
struct class_device *memnotify_device = NULL;
...
static int __init lowmemnotify_init(void)
{
	...
	memnotify_device = class_device_create(
...
error_create_class_file:
	class_device_del(memnotify_device);
